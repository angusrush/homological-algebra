...
\begin{definition}[additive category]
  ...
  A category $\mathcal{C}$ is \defn{additive} if it has products and is $\mathsf{Ab}$-enriched.
\end{definition}
...
\begin{definition}[direct sum]
  ...
  Let $\mathcal{C}$ be a category with products and coproducts which agree. Then we say that $\mathcal{C}$ has \defn{direct sums}, and denote the product/coproduct of $A$ and $B$ by $A \oplus B$.
\end{definition}
...
\begin{proposition}
  Let $\mathcal{C}$ be a category with finite direct sums (\hyperref[def:direct_sum]{Definition~\ref*{def:direct_sum}}).\footnote{I.e.\ with finite products and coproducts which agree, including a zero object.} Then $\mathcal{C}$ has a canonical $\Ab$-enrichment.
\end{proposition}
\begin{proof}
