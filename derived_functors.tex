...
\begin{definition}[Mittag-Leffler condition]
  ...
  Let \(\{A_{i}\}\) be a tower of abelian groups. We say that \(\{A_{i}\}\) satisfies the \defn{Mittag-Leffler condition} if for each \(n \geq 0\) there exists a \(N = N(n) > n\) such that the for all \(m \geq N\), the image of \(A_{m} \to A_{n}\) is equal to the image of \(A_{N} \to A_{n}\).

  We say that \(\{A_{i}\}\) satisfies the \defn{trivial Mittag-Leffler condition} if for all \(n \geq 0\) there exists \(N(n) > n\) such that \(\im(A_{N(n)} \to A_{n}) = 0\).
\end{definition}
