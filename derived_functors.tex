...
\begin{equation*}
  \colim F \cong \coprod_{i \in I} F(i) / E,
\end{equation*}
where $E$ is the subgroup generated by elements of the form $x - x'$, where $x \in F(i)$ and $x' \in F(i')$, such that there exists $j \in I$, $y \in F(j)$, and maps $u\colon j \to i$ and $u'\colon j \to i'$ such that $F(u)(y) = x$ and $F(u')(y) = x'$.
\begin{equation*}
...
\begin{proof}
  \leavevmode
  \begin{enumerate}
    \item A priori, we are only guaranteed that $a \in \colim A$ can be written as an equivalence class
      \begin{equation*}
        \left[ \sum_{i \in J} a_{i} \right],\qquad a_{i} \in A_{i},
      \end{equation*}
      for some finite subset $J \subset I$. Since $J = \{i_{1}, \ldots, i_{n}\}$ is finite, we can find an upper bound $j \geq i_{\ell}$ for all $\ell$. In the quotient,
...
\begin{proof}
  ...
  To this end, let $t \in \Hom_{\Ab^{I}}(A, B)$ be monic.
\end{proof}
...
Let $I$ be a small category, and let $\mathcal{A}$ be an abelian category such that for every set-indexed collection $\{A_{s}\}_{s \in S}$, the product $\prod_{s \in S} A_{s}$ exists. Then $\mathcal{A}^{I}$ has enough injectives.
...
\begin{equation*}
  R^{i} \lim := \Lim^{i} \colon A^{I} \to I.
\end{equation*}

For us it will be sufficient to consider the case in which $I = \N$ and $\mathcal{A} = \Ab$, so a functor $I \to \mathcal{A}$ is a string
\begin{equation*}
  \cdots \to A_{2} \to A_{1} \to A_{0}
\end{equation*}
of abelian groups.

We will call functors $\N \to \Ab$ \emph{towers.}
